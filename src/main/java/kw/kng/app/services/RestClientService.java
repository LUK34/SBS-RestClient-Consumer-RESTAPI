...
public class RestClientService 
{
	...
	public Ticket getTicketInfo(String ticketId)
	{
		...
		ResponseEntity<Ticket> re=rt.getForEntity(IRCTC_ENDPOINT_2, Ticket.class,ticketId);
		
		int statusCode=re.getStatusCodeValue();
		
		if(statusCode==200)
		{
			Ticket tc=re.getBody();
			return tc;
		}
		return null;
	}
	...
	{
		RestTemplate rt=new RestTemplate();
		ResponseEntity<Ticket> re=rt.postForEntity(IRCTC_ENDPOINT_1, passy, Ticket.class);
